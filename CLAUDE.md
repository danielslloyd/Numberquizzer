...
Assets are **WebP** (201KB total). Coins must keep an alpha channel for the disc mask; PNG cost 5x for the same result (406KB → 78KB on the coins), since palette-quantizing a photograph saves almost nothing. If you re-export, keep the tight crop and the uniform sizes — sloppy padding silently breaks the to-scale sizing, because the image's box *is* the piece.
